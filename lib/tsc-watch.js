...
tscProcess.stdout.on('data', buffer => {
  ...
  const errors = cleanLines.filter(line => typescriptErrorRegex.test(line) || typescriptPrettyErrorRegex.test(line));
  if (errors.length) {
    hadErrors = true;
  }
...
const Signal = {
  ...
  emitFail: (errors) => Signal.send('compile_errors', errors)
};
